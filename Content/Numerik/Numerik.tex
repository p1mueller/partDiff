...
$A^{(n)}\tilde{u}^{(n)} -f^{(n)} =0 $; exakt: $A^{(n)} u^{(n)} -f^{(n)} =
r^{(n)}=Residuenvektor$  $r^{(n)}=A^{(n)}(\tilde{u}^{(n)} - u^{(n)})$\\
Konsistent wenn $\lim\limits_{n\to \infty} ||r^{(n)}||_{\frac{1}{n}}=0$\\
...
Determinante: $|A|$ = Flächenfaktor: Wie viel mal grösser wird die Fläche\\
...
$\left(\frac{u(x+eh,y)-u(x,y)}{e(e+w)} +\frac{u(x+wh,y)-u(x,y)}{w(e+w)}
+ \frac{u(x+nh,y)-u(x,y)}{n(n+s)} + \frac{u(x+sh,y)-u(x,y)}{n(n+s)}\right) $\\$\frac{2}{h^2}= f(x,y)$
\\
oder
\\
...
Wenn $\delta_n u(x,y) = 0$, dann spricht man auch von der Spiegelmethode.
\end{minipage}
